...
- `cairn feedback "<message>"` to record cairn friction; entries land in `.cairn/feedback.md` for triage into native todos (`meta/todos/`) or upstream issues.
...
This repo's own development uses cairn's native Todo artefact, not beads, for
new work (`dec.native-todos-first`). See `AGENTS.md`'s "Task tracking" section
for the authoritative rules: `cairn todo new <slug> --node <id>`, `cairn todos
<node>`, `cairn status`. Historical bd issues remain readable via `bd show
<id>` for archaeology; do not create new ones for this repo's own work.
