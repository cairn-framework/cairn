...
3 files in `docs/`. All are at least partially outdated relative to the current phase spec structure in `openspec/specs/` and the campaign plan at `meta/campaigns/rust-full-spec.md`. One file (`docs/spec.md`) is now superseded as an implementation reference by the dedicated OpenSpec capability specs. One file (`docs/dsl.md`) is partially stale. One file (`docs/phase-2-deferrals.md`) is almost entirely stale and contradicts the current phase plan.
...
### `docs/dsl.md` — PARTIALLY_STALE (superseded in substance by `openspec/specs/parser/spec.md`)

- DSL grammar overview is consistent with `docs/spec.md` section 7 and `openspec/specs/parser/spec.md`.
...
- No mention of multi-path syntax (`path ["./a", "./b"]`), which spec.md section 7 requires the parser to handle — and `openspec/specs/parser/spec.md` scenario "Path list is normalized" tests for. `docs/dsl.md` predates this addition.
...
2. `docs/dsl.md` — add multi-path syntax example; replace "MVP" with "initial implementation" or similar; note it is a grammar overview supplement, not the authoritative parser spec.
