...
///
/// Candidates whose path is already declared in the blueprint are
/// filtered out so the delta contains only additions, renames (folded
/// to removal + addition), and removals. Returns `Ok(None)` when the
/// codebase and blueprint already agree and no change is written.
///
...
/// and stub contracts. The extraction is pre-filtered to new candidates only;
/// renames and removals were detected against the full discovery result.
///
/// # Errors
///
