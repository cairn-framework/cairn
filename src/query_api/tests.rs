...
#[test]
fn test_execute_get_includes_accepted_decision_pointers() {
    let tmp = std::env::temp_dir().join(format!("cairn-get-decisions-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&tmp);
    let _ = std::fs::create_dir_all(tmp.join("meta/decisions"));
    let _ = std::fs::write(
        tmp.join("cairn.blueprint"),
