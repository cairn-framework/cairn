//! Registry loader and parser tests.
...
status: accepted\n\
date: 2026-04-01\n\
refined_by: [dec.authored]\n\
---\n\
# Decision\n",
    )?;
    fs::write(
        root.join("meta/decisions/refiner.md"),
        "---\n\
id: dec.refiner\n\
nodes: [app.api]\n\
status: accepted\n\
...
# Refiner\n",
    ...
    let base = set
        .decisions
        .iter()
        .find(|decision| decision.id == "dec.api")
        .expect("base decision loaded");
    assert_eq!(base.refined_by, vec!["dec.refiner"]);
    assert_eq!(base.superseded_by, vec!["dec.refiner"]);
    assert!(!base.refined_by.iter().any(|id| id == "dec.authored"));
    Ok(())
}

#[test]
fn test_load_artefacts_reports_unknown_node_references() -> Result<(), Box<dyn std::error::Error>> {
