...
- Reconciled `.beads/issues.jsonl` with completed work by closing 19 stale open beads whose acceptance criteria were already satisfied on disk and verified by passing pre-archive rust gates.
  ...
  - Commit: 859bda1
- Added bead workflow notes to `CLAUDE.md` in a standalone section (outside the bd-managed block) documenting the `bd close && bd export` pattern and the bare-close-loop pitfall.
  - Commit: b15b529
