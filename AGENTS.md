...
Cairn's **native Todo artefact** (`docs/spec.md` §8.2, "Todo (authority)") remains the shipped artefact authority and is unchanged. bd does not replace it: bd issues carry a `cairn-node:<id>` label and surface per node via `cairn backlog <node>` (the read-only beads-per-node view, `src/state/backlog.rs`) without minting Todo files. (`--include-todos` on `cairn neighbourhood` is a separate surface that renders native Todo artefacts, not beads.)

The two "Beads Issue Tracker" blocks below were generated by `bd setup`; their task-tracking Rules lines have been **reconciled in place** to match this section (bd is this repo's chosen tracker, not a universal mandate over cairn's native Todo). They are now hand-maintained: do not blindly re-run `bd setup`, which would regenerate the universal-mandate wording ("do NOT use TodoWrite", "no markdown TODO lists"). If a tool regenerates them anyway, re-apply this reconciliation. Narrowing the universal mandate implements `dec.native-task-state-and-agent-guidance` ruling 2; editing the blocks in place rather than regenerating them is the "pin-or-regenerate" approach that decision records as a revisit trigger.
