...
### Requirement: Phase 7.7 acceptance criteria have failing ignored tests

Phase 7.7 UX Foundation acceptance-criterion scenarios SHALL each have a corresponding `#[test]` in `tests/phase_7_7_ux_foundation.rs` marked `#[ignore = "awaits phase-7.7"]`. The pre-phase archives on a green `cargo test` because the tests are skipped. Phase 7.7 removes each `#[ignore]` attribute as the corresponding feature code lands.
...
- **THEN** all 28 tests in `tests/phase_7_7_ux_foundation.rs` fail with `todo!()` panics
...
- **THEN** no `#[ignore = "awaits phase-7.7"]` attribute remains in `tests/phase_7_7_ux_foundation.rs`
