...
Add `tests/phase_7_8_cairn_export.rs` containing one `#[test]` per phase-7.8 acceptance-criterion scenario, each annotated `#[ignore = "awaits phase-7.8"]`. The tests use `todo!()` bodies so they compile but fail when run. The file compiles cleanly under all strict Rust gates. This pre-phase archives on a green `cargo test` because all new tests are skipped.

## Acceptance Criteria

- `tests/phase_7_8_cairn_export.rs` exists and compiles.
- The file contains exactly one `#[test]` per phase-7.8 scenario, each marked `#[ignore = "awaits phase-7.8"]`.
