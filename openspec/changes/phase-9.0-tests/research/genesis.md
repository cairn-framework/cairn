...
- Stub count: from 17 (10 acceptance + 7 heuristic) to 32 (25 acceptance + 7 heuristic). The acceptance count exceeds the 23-scenario count because Req 5 scenario 1 carries three load-bearing assertions (queue write, pending state, trace phase) split across three stubs for clear failure isolation; the other 22 scenarios map one-to-one. The headline metric the proposal cites is the 23-scenario / 8-requirement shape mirroring the brownfield spec.
...
A: Yes. The original tasks.md ended at section 7 (Required Verification). Wave 4 task groups slot in as 7 (Suggest), 8 (Interview), 9 (Templates), 10 (Obligations); verification renumbers to 11. Task IDs continue the `<group>.<task>` pattern. The Makefile's `[X]` checkbox detector (per `CLAUDE.md`) treats each `- [ ]` line uniformly regardless of section number, so the renumber is mechanically safe.
