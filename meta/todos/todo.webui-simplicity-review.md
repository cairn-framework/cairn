...
Note: the webui's state architecture (2,013-line `app.js` monolith with scattered component-local `useState`, no central store) is a known modularity concern tracked under `todo.webui-feature-module-split` (filed by the completed `todo.architecture-modularity-audit`, which measured coupling, change-hotspots, and parallel-edit boundaries and chose feature-local modules over a global TEA store). Do not assume the fix is a single global store (that can just relocate the merge hotspot). Keep this simplicity-review todo focused on the UX progressive-disclosure work it already scopes.
